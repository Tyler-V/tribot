package scripts.clay_miner.paint;
...
public class ClayMinerPaint extends Painter {
	...
	public ClayMinerPaint() {
		...
		getPaint().add(new PaintContainer(PaintColumns.AUTO, new Rectangle(7, 371, 506, 75), TEXT_COLOR, TEXT_BACKGROUND_COLOR, BORDER_COLOR,
				...
				new PaintBoxString(() -> "Soft clay: " + Strings.format(Vars.get().clay) +
						" " +
						Vars.get().getSkillsTracker().getAmountPerHourString(Vars.get().clay)),
				new PaintBoxString(() -> "Profit: " + Strings.format(Vars.get().profit) +
						" " +
						Vars.get().getSkillsTracker().getAmountPerHourString(Vars.get().profit)),
				new PaintBoxString(() -> "Mining Level: " + Skills.getActualLevel(SKILLS.MINING) +
						" (+" +
						Vars.get().getSkillsTracker().getLevelsGained(SKILLS.MINING) +
						")"),
				new PaintBoxString(() -> "XP Gained: " + Vars.get().getSkillsTracker().getXPGained(SKILLS.MINING) +
						" " +
						Vars.get().getSkillsTracker().getXPPerHourString(SKILLS.MINING))));
		getPaint().add(new PaintToNextLevel(Vars.get().getSkillsTracker(), SKILLS.MINING, new Rectangle(12, 449, 496, 20), BORDER_COLOR));
	}
