 ...
 *
 * Elements can be added using the <code>add(...)</code> methods which accept instances of {@link JsonValue}, strings, primitive numbers, and boolean values. To
 * replace an element of an array, use the <code>set(int, ...)</code> methods.
 * </p>
 *
 * Elements can be accessed by their index using {@link #get(int)}. This class also supports iterating over the elements in document order using an
 ...
 * </p>
 *
 * Note that this class is <strong>not thread-safe</strong>. If multiple threads access a <code>JsonArray</code> instance concurrently, while at least one of
 * these threads modifies the contents of this array, access to the instance must be synchronized externally. Failure to do so may lead to an inconsistent
 * state.
 * </p>
 *
...
public class JsonArray extends JsonValue implements Iterable<JsonValue> {
     ...
     * @throws UnsupportedOperationException if the input does not contain a JSON array
     */
    public static JsonArray readFrom(String string) throws IOException {
     ...
     *
     * The returned JsonArray is backed by the given array and reflects subsequent changes. Attempts to modify the returned JsonArray result in an
     ...
     * @return the array itself, to enable method chaining
     * @throws IndexOutOfBoundsException if the index is out of range, i.e. <code>index &lt; 0</code> or <code>index &gt;= size</code>
     */
    ...
    /**
     * Returns a list of the values in this array in document order. The returned list is backed by this array and will reflect subsequent changes. It cannot be
     * used to modify this array. Attempts to modify the returned list will result in an exception.
     *
