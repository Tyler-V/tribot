...
public final class AntiBan {
    ...
    /**
     * Gets the sleeping seed for the method sleep().
     *
     * @return The sleeping seed.
     */
    public static double getSleepSeed() {
        return getSleepSeed(Client.getUsername());
    }

    /**
     * Gets the sleeping seed for the method sleep() with the specified string.
     *
     * @param string The string.
     * @return The sleeping seed.
     */
    public static double getSleepSeed(String string) {
        Random random = new Random(Strings.charSum(string));
        for (int i = 0; i < string.toCharArray().length; i++) {
            random.nextDouble();
        }
        double d = random.nextDouble();
        if (d < .1) { //If the seed generated is too low, bump it up.
            d = d + .1;
        }
        return d;
    }
    ...
    public static void sleepReactionTime() {
        ...
        Timer sleepTimer = new Timer(reaction_time / 2); //Since they are too long.
        sleepTimer.start();
        while (!sleepTimer.timedOut()) {
            if (AntiBan.shouldEat()) {
                debug("Break-out condition interrupted sleep");
                break;
            }
            sleep();
        }
     ...
     */
    public static void generateTrackers(int estimated_wait, boolean underAttack) {
    ...
    /**
     * Checks to see if the player should eat.
     *
     * @return True if the player should eat, false otherwise.
     */
    ...
    public static boolean eat(String option, RSItem item) {
        ...
        //Get the current hp to track it.
    ...
    /**
     * Sleeps the current thread for a time in milliseconds generated based on the tribot username (will always be under 100ms).
     */
    public static void sleep() {
        Client.sleep(0, (int) (getSleepSeed() * 100));
    }

    /**
     * Sleeps the current thread for a time in milliseconds generated based on the tribot username (will always be under 100ms) multiplied by the
     * specified iterations.
     * This method does not guarantee a static sleep time each iteration.
     *
     * @param iterations How many times to sleep.
     * @see #sleep()
     */
