...
public class FxLoader extends Application {
	...
	this.fxml = !local ? fxml
		: script.getResource(fxml.toString().split("/")[fxml.toString().split("/").length - 1]);
	if (css != null)
	    this.css = !local ? css
		    : script.getResource(css.toString().split("/")[css.toString().split("/").length - 1]);
    ...
    public FxLoader(Class<?> script, URL fxml, boolean local) {
	this(script, fxml, null, local);
    }
