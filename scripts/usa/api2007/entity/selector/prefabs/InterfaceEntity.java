...
import scripts.usa.api2007.entity.selector.Utils;

/**
 * @author Laniax
 */
public class InterfaceEntity extends scripts.usa.api2007.entity.selector.FinderResult<RSInterface> {
	...
	/**
	 * Finds all the interfaces whose text do not contain any of the specified
	 * texts. Case insensitive.
	 *
	 * @param text
	 * @return
	...
	/**
	 * Finds all the interfaces that have actions
	 *
	 * @param actions
	 * @return
	 */
	public InterfaceEntity hasActions() {
		...
		filters.add((rsInterface) -> {
			...
			return interfaceActions.length > 0;
		});
	...
	public InterfaceEntity actionEquals(String... actions) {
		...
		filters.add((rsInterface) -> {
			...
			List<String> searchList = Utils.stringListToLowercase(Arrays.asList(actions));

			for (String action : interfaceActions) {

				// Weird, looks like RSInterface#getActions() might return an
	...
	/**
	 * Finds all the interfaces whose action do not contain any of the specified
	 * actions. Case insensitive.
	 *
	 * @param actions
	 * @return
	...
	/**
	 * Finds all the interfaces whose text colour equals any of the specified
	 * colours.
	 *
	 * @param colours
	 * @return
	...
	/**
	 * Finds all the interfaces whose text colour does not equals any of the
	 * specified colours.
	 *
	 * @param colours
	 * @return
	...
	/**
	 * Finds all interfaces which are substantiated.
	 * 
	 * @return
	 */
	public InterfaceEntity isSubstantiated() {

		filters.add(inter -> Interfaces.isInterfaceSubstantiated(inter));
	...
	public RSInterface[] getResults() {
		...
		if (selectedMaster != null) {
			...
			// If a child was set as well. We will search over the child and its
			// components.
			if (selectedChild != null) {
				...
				// Only a master was set? fine. We search over it, its children
				// and the children's components.
		...
		// If we didn't specify a master and optionally a child. We simply
