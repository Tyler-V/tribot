...
public class RSWorld {
	...
	public final static int[] WORLDS_NOT_SUPPORTED = new int[] { 7, 8, 15, 16,
			23, 24, 31, 32, 47, 48, 55, 56, 63, 64, 71, 72 };
	public final static int WORLD_MAXIMUM_END = 79;
		...
		while (true && timeout > System.currentTimeMillis()) {
			if (Login.getLoginState() == STATE.INGAME) {
				...
				if (hasMisconfiguredWorldSettings()) {
					...
					Mouse.clickBox(clickArea.x, clickArea.y, clickArea.x
							+ clickArea.width, clickArea.y + clickArea.height,
							0);
					Timing.waitCondition(new Condition() {
						@Override
						public boolean active() {
	...
	public static int getRandomWorld() {
		int randomWorld;
		while (!isWorldSupported((randomWorld = General.random(1,
				WORLD_MAXIMUM_END))))
			;
